...
. 這些 VM 類型使用 https://docs.microsoft.com/en-us/azure/virtual-machines/windows/disks-enable-ultra-ssd["超SSD"^] for VNVRAM 、提供更好的寫入效能。
+
...
. 使用單一節點系統時、所有執行個體類型都支援高速寫入。您可以在部署期間或之後的任何時間、從 BlueXP  啟用高速寫入速度。 https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/concept-write-speed.html["深入瞭解如何選擇寫入速度"^]。
. 使用SSD時可啟用增強的寫入效能。
...
| 最大系統容量（磁碟+物件儲存） | 2 TiB ^1^ | 10. TiB | 368 TiB | 每個授權368TiB 
...
如需 Azure 區域支援 https://cloud.netapp.com/cloud-volumes-global-regions["Cloud Volumes全球區域"^]、請參閱。
